...
func verifyAdminCredentials(db *sql.DB, admin Admin) (bool, error) {
	var storedPasswordHash string
	err := db.QueryRow("SELECT password FROM admins WHERE username = $1", admin.Username).Scan(&storedPasswordHash)
	if err != nil {
		if err == sql.ErrNoRows {
...
func RegisterAdminHandler(db *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		_, err = db.Exec("INSERT INTO admins (username, password) VALUES ($1, $2)", admin.Username, string(hashedPassword))
		if err != nil {
